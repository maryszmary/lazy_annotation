...
def get_tiernames(lines):
    ...
    return tier_words, tier_sounds, tier_stops, tier_diffwords


def get_intr(lines):
    intr = []
    for el in lines:
        if el != tier_diffwords: # commented out: 27.01.17 (UNCOMMENTED BACK TEMPORARY)
...
tier_words, tier_sounds, tier_stops, tier_diffwords = get_tiernames(lines)
...
intr = get_intr(lines)
all_sets = get_all_sets(lines)

#аннотация слов

def word_annotation(all_sets, tier_words):
    for arr in all_sets:
        if arr[2] != '\"\"':
            i = 0
            ii = 1
        ...
        if arr[w] == tier_sounds:
            ...
            if thesound != u'' and thesound in STOPS:
                if tier_stops not in arr[w + 7:]:
                    print("SOMETHING GONE WRONG at " + str(w))
                    print(u'\n'.join(arr[w:]))
                    print("UTTERLY")
